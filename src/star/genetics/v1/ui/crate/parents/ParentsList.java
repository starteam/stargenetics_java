...
public class ParentsList extends ParentsList_generated implements FocusListener, ParentsListInterface
{
	private static final long serialVersionUID = 1L;
	private boolean matingEnabled = true;
	private boolean mated = false;
	private ParentsList self = this;

	public ParentsList(CrateModel model, boolean matingEnabled)
	{
		setCrateModel(model);
	...
	{
		super.addNotify();
		setListModel(new star.genetics.v1.ui.impl.ParentsListModelImpl(getCrateModel()));
	...
	public void setCreature(Creature c)
	{
		if (c != null)
		{
			Object q = getListModel().get(c.getSex());
			if (q != null && q instanceof Creature)
			{
				int ok = JOptionPane.showConfirmDialog(this, MessageFormat.format(Messages.getString("ParentsList.0"), String.valueOf(c.getSex()).toLowerCase()), Messages.getString("ParentsList.1"), JOptionPane.YES_NO_OPTION); //$NON-NLS-1$ //$NON-NLS-2$
	...
	void onMate(CrateMateRaiser r)
	{
		setTransferHandler(new TransferHandler()
		{
			...
			{
				JOptionPane.showMessageDialog(self, Messages.getString("ParentsList.2")); //$NON-NLS-1$
				return false;
			}
		});
		mated = true;
		...
		if (getSelectedCreature() != null)
		{
			...
			if (!mated && matingEnabled)
			{
				Tuple removeTuple = new Tuple(6, Action.REMOVE, new Runnable()
				{
					public void run()
					{
						int ret = JOptionPane.showConfirmDialog(getJList(), MessageFormat.format(Messages.getString("ParentsList.3"), getSelectedCreature().getName()), Messages.getString("ParentsList.4"), JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE); //$NON-NLS-1$ //$NON-NLS-2$
