...
public class WebSamples extends WebSamples_generated
{
	...
	private String getAlternateText()
	{
		...
		{
			isAlternate = true;
			samplesURL = alternate_url;
			text = new String(FileUtils.getStreamToByteArray(samplesURL.openStream()));
		}
		catch (Throwable t2)
		{
			samplesURL = null;
	...
	private String getText()
	{
		...
		try
		{
			...
			text = new String(FileUtils.getStreamToByteArray(c.getInputStream()));
		}
		catch (Throwable t)
		{
	...
	void openURL()
	{
		...
		// }
		// }).start();
		// JOptionPane.showMessageDialog(self, "Attempting to open external browser. External URL is: " + openURL);
		// setVisible(false);
		// dispose();
				...
				if (e.getEventType() == EventType.ACTIVATED)
				{
					...
					if (url != null)
					{
						...
						if (modelFileName.endsWith(".sgz") || modelFileName.endsWith(".xls")) //$NON-NLS-1$ //$NON-NLS-2$
						{
							try
							{
								modelFileName = label.getText(e.getSourceElement().getStartOffset(), e.getSourceElement().getEndOffset() - e.getSourceElement().getStartOffset());
							}
							catch (Throwable t)
							{
							...
							new Thread(new Runnable()
							{
								public void run()
								{
									try
									{
										ProgressMonitorInputStream pis = new ProgressMonitorInputStream(self, MessageFormat.format(Messages.getString("WebSamples.0") , modelFileName), url.openStream()); //$NON-NLS-1$
										pis.getProgressMonitor().setMillisToPopup(0);
										pis.getProgressMonitor().setMillisToDecideToPopup(0);
										bytes = FileUtils.getStreamToByteArray(new BufferedInputStream(pis));
										modelURL = url;
