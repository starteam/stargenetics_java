...
public class Utilities
{
	public static void preload()
	{
		Thread t = new Thread(new Runnable()
		{
			public void run()
			{
				try
				{
					Utilities.log("Utilities perload start "); //$NON-NLS-1$
					(new Load().load(Load.class.getClassLoader().getResourceAsStream("resources/xls/SpeedInit.xls"))).toString(); //$NON-NLS-1$
					Utilities.log("Utilities perload end "); //$NON-NLS-1$
				}
				catch (Throwable t)
				{
					t.printStackTrace();
	...
	{
		System.err.println(str + ":" + (System.nanoTime() - startup) / 1000000.0 + "ms."); //$NON-NLS-1$ //$NON-NLS-2$
	}
}
