	...
	public static Creature.Sex getSex(GeneticMakeup makeup, Genome genome, String name) throws ParseException
	{
		if (genome.getSexType() == Genome.SexType.XY)
		{
			...
			if (zeros == 0)
			{
				...
				for (Gene g : c2.getGenes())
				{
					...
					{
						throw new ParseException(MessageFormat.format(Messages.getString("Sex.4"), name)); //$NON-NLS-1$
					}
				}
				return Creature.Sex.FEMALE;

			}
			throw new ParseException(MessageFormat.format(Messages.getString("Sex.5"), name)); //$NON-NLS-1$
		}
		else if (genome.getSexType() == Genome.SexType.XO)
		{
			...
			for (Gene g : c2.getGenes())
			{
				...
				{
					throw new ParseException(MessageFormat.format(Messages.getString("Sex.7"), name)); //$NON-NLS-1$
				}
			}
			if (xx == 0 && x == 0)
			{
				throw new ParseException(MessageFormat.format(Messages.getString("Sex.8"), name)); //$NON-NLS-1$

			}
			if (xx != 0 && x != 0)
			{
				throw new ParseException(MessageFormat.format(Messages.getString("Sex.9"), name)); //$NON-NLS-1$
			}
